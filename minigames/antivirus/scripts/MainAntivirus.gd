...
var arrow_original_scale: Vector2 
...
func _input(event: InputEvent) -> void:
	if not game_active: return
	if event.is_action_pressed("ui_up"): current_direction = 0; _update_arrow_visuals()
	elif event.is_action_pressed("ui_right"): current_direction = 1; _update_arrow_visuals()
	elif event.is_action_pressed("ui_down"): current_direction = 2; _update_arrow_visuals()
	elif event.is_action_pressed("ui_left"): current_direction = 3; _update_arrow_visuals()

func _update_arrow_visuals(instant: bool = false):
	var target_rot = (current_direction * 90) + INDEX_ROTATION_OFFSET
	if instant: arrow.rotation_degrees = target_rot
	else:
		var t = create_tween()
...
func _handle_impact(item: IncomingItem, index: int):
	if item.type == "virus":
		...
		else: _on_safe_passed(item.node)
	active_items.remove_at(index)

func _on_virus_blocked(node: Node2D):
	if sfx_player:
		if audio_virus_block: sfx_player.stream = audio_virus_block
		sfx_player.play()
	var t = create_tween()
	t.tween_property(arrow, "scale", arrow_original_scale * 1.3, 0.05)
	t.tween_property(arrow, "scale", arrow_original_scale, 0.05)
	node.queue_free()

func _on_safe_passed(node: Node2D):
	if sfx_player:
		if audio_safe_pass: sfx_player.stream = audio_safe_pass
		sfx_player.play()
	var t = create_tween()
